...
pasos:[
 {t:"info", eti:"No adivinar", h:"El método",
  c:`<p>«La API no conecta con la base de datos». En lugar de cambiar cosas al azar, recorre las capas desde el origen:</p>
     <div class="diag">1. tengo red?              ip addr, ip route
2. llego a la maquina?      ping (si ICMP esta permitido), traceroute
3. resuelve el nombre?      dig / nslookup / getent hosts
4. el puerto esta abierto?  nc -vz host puerto, o curl
5. responde la aplicacion?  curl -v, logs del servicio
6. y en el destino?         ss -tlnp (escucha? en que IP?), cortafuegos</div>
