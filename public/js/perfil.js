  ...
  F.pintar(
    '<div class="ancho-texto">' +
      ...
      '<section class="seccion"><div class="seccion-cab"><h2>Apariencia</h2></div><div class="panel panel-pad" style="display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap"><span>Tema de la interfaz</span>' +
        '<div class="segmentado" id="seg-tema"><button data-t="claro" aria-pressed="' + (tema === "claro") + '">' + F.icono("sol").replace("<svg", '<svg style="width:14px;height:14px;vertical-align:-2px"') + ' Claro</button><button data-t="oscuro" aria-pressed="' + (tema === "oscuro") + '">' + F.icono("luna").replace("<svg", '<svg style="width:14px;height:14px;vertical-align:-2px"') + ' Oscuro</button><button data-t="sistema" aria-pressed="' + (tema === "sistema") + '">Sistema</button></div></div></section>' +
